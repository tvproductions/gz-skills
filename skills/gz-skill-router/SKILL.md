...
| Goal | Skill | Boundary |
| --- | --- | --- |
| Reduce persistent agent-instruction weight | `gz-agent-context-diet` | Preserves binding rules while pruning or disclosing context. |
| Review Python for operating-system assumptions | `gz-cross-platform-python` | Applies to Python portability, not general cross-platform product design. |
| Commit and publish a guarded save point | `gz-git-sync` | Explicit-only; commits and pushes after repository gates pass. |
| Compare delivered behavior with its owning intent | `gz-intent-audit` | Diagnoses and routes gaps; does not implement corrections unless separately requested. |
| Check intent, scope, and plan alignment | `gz-plan-audit` | Runs before implementation and audits existing artifacts. |
| Run the repository's complete verification | `gz-quality-gate` | Uses project-owned commands and reports unavailable dimensions honestly. |
| Audit or repair repository hygiene | `gz-repository-hygiene` | Begins read-only; cleanup requires authorization from the request or project workflow. |
| Preserve or resume engineering state | `gz-session-handoff` | Explicit-only; creates or consumes a durable continuity artifact. |
| Survey and prioritize technical debt | `gz-tech-debt-review` | Produces an evidenced report without fixing findings. |
| Refresh project dependencies and pinned tools | `gz-update-dependencies` | Covers project-managed versions, not machine-wide or deployed infrastructure. |

## Common sequences

- Dependency maintenance: `gz-update-dependencies` → `gz-git-sync` when the
  verified update should be published.
- Pre-publication confidence: `gz-quality-gate` → `gz-git-sync` when no broader
  maintenance workflow already ran the complete gate.
- Plan integrity: `gz-plan-audit` before implementation; `gz-intent-audit` after
  delivery when fulfillment is uncertain.
- Repository maintenance: `gz-repository-hygiene` → `gz-quality-gate` when the
  hygiene workflow did not already include the full final gate.
- Session continuity: `gz-session-handoff` at a stopping boundary; invoke it
